  ...
  if (transactionRaw) {
    ...
    if (EIP1559ShouldBeUsed(mainAccount.currency)) {
      ...
      strategies.forEach(strategy => {
        ...
        if (oldMaxPriorityFeePerGas && strategyMaxPriorityFeePerGas) {
          strategy.disabled =
            strategy.disabled ||
            strategyMaxPriorityFeePerGas.isLessThan(BigNumber(oldMaxPriorityFeePerGas));
        }
      });
    } else {
      ...
      if (oldGasPrice) {
        strategies.forEach(strategy => {
          strategy.disabled =
            strategy.disabled || strategy.amount.isLessThan(BigNumber(oldGasPrice));
        });
      }
    }
  }
